"""
O5（3連複オッズ）レコードのパーサー
JV-Data仕様書 １１．オッズ5（3連複）に基づく（12293バイト）

3連複の確定オッズを analytics.odds_final 用に返す。
データ区分 3=最終, 4=確定, 5=確定(月曜) のみ処理。
3連複は3頭の組み合わせ・順番不問（combination: "3-7-12" 昇順）
"""
...
def parse_o5(raw: str) -> list[dict] | None:
    """
    O5レコードをパースして analytics.odds_final 用の辞書リストを返す。
    確定オッズ（データ区分 3,4,5）のみ。3連複。
    """
    try:
        b = _to_bytes(raw)
    except Exception:
    ...
    # 3連複オッズ: pos 41, 816回×15B (組番6, オッズ6, 人気3) 組番昇順 01-02-03～16-17-18
    for i in range(816):
        base = 41 + i * 15
        combo_raw = _sub_bytes(b, base, 6)
        odds_s = _sub_bytes(b, base + 6, 6)
        pop_s = _sub_bytes(b, base + 12, 3)
        ...
        try:
            ...
            if a < 1 or b_val < 1 or c < 1 or a == b_val or b_val == c or a == c:
                continue
            combination = "-".join(str(x) for x in sorted([a, b_val, c]))
