"""
KS（騎手マスタ）レコードのパーサー
JV-Data仕様書 １４．騎手マスタ に基づく（位置は1始まりのバイト位置）
...
def parse_ks(raw: str) -> dict | None:
    """
    KSレコードをパースして analytics.jockeys 用の辞書を返す。
    ...
    try:
        ...
        jockey_id = _sub_bytes(b, 12, 5)
        if not jockey_id or jockey_id == "00000":
            return None
        ...
        name_kana = _sub_bytes(b, 110, 30) or None
        birth_date = _date_or_none(_sub_bytes(b, 34, 8))
        belong_code = _sub_bytes(b, 231, 1)
